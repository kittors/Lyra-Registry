...
打包本身是确定的——mtime、uid、gid 全部钉死，条目按路径排序，所以同一个 commit 打出来的 tar 逐字节相同（在 Node 和 workerd 两个运行时上都验过，都是 `0b62b719e245…`）。**但 gzip 不是**：同一份 tar 交给两个运行时压，大小一样、哈希不同。所以条目里那个 sha256 标识的是「这次构建」，够用来校验下载；要问「是不是同一份内容」，比的是里面那层 tar。
